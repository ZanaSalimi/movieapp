...
export class Search extends Component {
    ...
    onChange = (e) => {
        ...
        if(this.state.search.length >= 1){
            fetch(`https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&query=${this.state.search}&page=1&include_adult=false`)
            .then(res => res.json())
            .then(data => {
                this.setState({ options: data.results.map((option) => option.title) })
            })
        }
    }
    onSubmit = (e) => {
